...
const double Lf =
    2.67;  // this is the length from front of vehicle to Center-of-Gravity
const double VELOCITY_MAX =
    100.0;  // this is what we ideally want our speed to always be
...
const int NX =
    N * NUMBER_OF_STATES +
    (N - 1) * NUMBER_OF_ACTUATIONS;   // number of state + actuation variables
...
const double W_ddelta = 150.0;  // weight cost for high difference between
                                // consecutive steering actuations
const double W_da = 15.0;       // weight cost for high difference between
                                // consecutive acceleration actuations
...
class MPC {
 public:
  ...
  Dvector g_lowerbound;  // value constraint for each corresponding constraint
                         // expression
  Dvector g_upperbound;  // value constraint for each corresponding constraint
                         // expression
  ...
  // this function solves the model given the current state and road curve
  // coefficients.
